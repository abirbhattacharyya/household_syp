      ...
      module SingletonMethods
        ...
        def suggest(fields, word, treshold = nil)
          ...
          if fields.kind_of?(Array) 
            ...
            search_results = self.find(:all, :conditions => ["#{fields} Rlike ?", word])
          end
       
          # Checks if +word+ exist in the requested field(s)
          puts "Search Result :  #{search_results.size}"
...
class String
  ...
  def distance(other,record)
    #Text::Levenshtein::distance(self, other)
    Text::Levenshtein::distance(record, other)
  end
  ...
  def similar?(other, threshold = 2)
    ...
    success = false
    str.each do |s|
      result = self.distance(other.downcase,s.downcase) <= threshold ? true : false
      if result == true
        success = true
        break
      end
